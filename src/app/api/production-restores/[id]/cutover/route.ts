...
import { cutoverProductionRestore } from "@/features/restore/production-restore-service";
...
import { productionRestoreCutoverSchema } from "@/lib/validation/api";
...
export async function POST(
  ...
  try {
    ...
    const body = productionRestoreCutoverSchema.safeParse(await request.json());
    ...
    const restore = await cutoverProductionRestore({
      restoreId: id,
