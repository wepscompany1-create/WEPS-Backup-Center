...
- Open production sessions are not terminated by Backup Center. Operators must stop the source application and drain its pools. Active-connection failures stay retryable in `AWAITING_CUTOVER`; external cutover is used only when rename is denied by permission or provider policy.
